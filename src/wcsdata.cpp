	...
	try {
		if (unit.count("NAXIS") > 0)
			if (unit.header("NAXIS") == "2") {
				std::array<float, 4> m{};
				const std::array<float, 2> ref_px{ unit.header("CRPIX1").toFloat(), unit.header("CRPIX2").toFloat() };
				if (unit.count("CD1_1") > 0 && unit.count("CD1_2") > 0 &&
					...
					const std::array<float, 2> c_delt{ unit.header("CDELT1").toFloat(), unit.header("CDELT2").toFloat() };
